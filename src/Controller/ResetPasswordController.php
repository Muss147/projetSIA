...
class ResetPasswordController extends AbstractController
{
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer, TranslatorInterface $translator): RedirectResponse
    {
        ...
        if (!$user) {
            $this->addFlash('reset_password_error', 'Désolé, cette adresse email ne se trouve pas dans notre base de données. Veuillez vérifier à nouveau l\'email.');
            return $this->redirectToRoute('app_forgot_password_request');
        }
        ...
        $email = (new TemplatedEmail())
            // ->from(new Address('hello@moussa-fofana.com', '3coeurs'))
            ->from(new Address('hello@moussa-fofana.com'))
